...
$$
A^TA = V\Sigma^TU^T \cdot U \Sigma V^T \newline U^T\cdot U = I, \therefore A^TA = V^T\Sigma^T\Sigma V
$$
...
반대로 n < m의 경우 $$A^+$$는 우측역행렬이 된다. n < m(파라미터 >  데이터)은불충분결정평면 (underdetermined) 상태를 의미한다. 회귀에서 자주 보이는 유형은 아니나, 딥러닝에서는 이러한 형태의 행렬벡터가 빈번히 등장한다. 딥러닝의 feature(파라미터)는 수억수천만개이지만 학습시킨 데이터는 이 수준에는 못 미치기 때문이다.

$$
A^+ = A^T(AA^T)^{-1} \newline AA^+ = AA^T(AA^T)^{-1} \newline AA^+ = (AA^T)(AA^T)^{-1} = I \newline \therefore AA^+ = I
$$
